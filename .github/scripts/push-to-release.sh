...
# Usage: ./push-to-release.sh <files-pattern> <commit-message> [branch-name]
...
FILES_PATTERN="$1"
COMMIT_MESSAGE="$2"
BRANCH_NAME="${3:-update-release}"  # default update-release branch
...
if git ls-remote --heads origin "${BRANCH_NAME}" | grep -q "${BRANCH_NAME}"; then
    ...
    if [[ "${CURRENT_BRANCH}" == "${BRANCH_NAME}" ]]; then
        echo "Already on branch ${BRANCH_NAME}, switching to release/1.0.0 first"
        git checkout release/1.0.0
    fi
    
    git branch -D "${BRANCH_NAME}" 2>/dev/null || true    
    git checkout -b "${BRANCH_NAME}" origin/release/1.0.0
    
    echo "Created fresh branch '${BRANCH_NAME}' from release/1.0.0 to avoid conflicts"
else
...
if command -v gh &> /dev/null; then
    ...
    EXISTING_PR=$(gh pr list --head "${BRANCH_NAME}" --base release/1.0.0 --json number --jq '.[0].number' 2>/dev/null || echo "")
    ...
    echo "Creating new PR to release/1.0.0 branch..."
