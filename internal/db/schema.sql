...
create index if not exists idx_source_types_source_id on source_types (source_id);

-- source_list_types: blocklist/allowlist per source type
create table if not exists source_list_types (
  id integer primary key,
...
create index if not exists idx_source_list_types_type_id on source_list_types (source_type_id);

-- source_list_type_groups: size group membership for a list type
create table if not exists source_list_type_groups (
  source_list_type_id integer not null references source_list_types (id) on delete cascade,
  group_name text not null,
  primary key (source_list_type_id, group_name)
) strict, without rowid;

create index if not exists idx_source_list_type_groups_list_type_id on source_list_type_groups (source_list_type_id);
...
create index if not exists idx_source_countries_source_id on source_countries (source_id);
...
create index if not exists idx_entries_lookup on entries (entry, generic_source_type, list_type);

create index if not exists idx_entries_source on entries (source_id, generic_source_type);

create index if not exists idx_entries_source_type_list on entries (source_id, actual_source_type, list_type);
...
create index if not exists idx_entry_groups_source_id on entry_groups (source_id);
...
create index if not exists idx_entry_categories_source_id on entry_categories (source_id);
...
create index if not exists idx_consolidated_lookup on consolidated_entries (
  entry,
  generic_source_type,
  list_type,
  consolidation_type
...
create index if not exists idx_consolidated_type on consolidated_entries (
  consolidation_type,
  generic_source_type,
  list_type
);
...
create table if not exists overlap_results (
  id integer primary key,
...
create index if not exists idx_overlap_source on overlap_results (source_name, generic_source_type);

-- top_entries: entries appearing in N+ sources
create table if not exists top_entries (
  id integer primary key,
  entry text not null,
  generic_source_type text not null,
  list_type text not null,
...
create index if not exists idx_top_entries_lookup on top_entries (generic_source_type, list_type, min_sources);