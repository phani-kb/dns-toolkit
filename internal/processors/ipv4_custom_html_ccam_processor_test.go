...
func TestIpv4CustomHtmlCcamProcessor_Process(t *testing.T) {
...
<tr><td>Tayuya</td><td>01/05/2018 20:03:30</td><td>brausincsystem.pro</td><td>db58ba784b92dc64ff7e4a28078305de7117919d</td></tr></table>`
	...
	processor := NewIpv4CustomHtmlCcamProcessor("ipv4_custom_html_ccam", "blocklist")
	valid, invalid := processor.Process(logger, sampleContent)

	expected := []string{"103.208.86.48", "185.99.133.162", "69.73.130.134"}
	if len(valid) != len(expected) {
		t.Errorf("expected %d valid IPs, got %d: %v", len(expected), len(valid), valid)
	}
	for i, ip := range expected {
		if i >= len(valid) || valid[i] != ip {
			t.Errorf("expected valid[%d]=%q, got %q", i, ip, valid[i])
		}
	}
	if len(invalid) == 0 {
		t.Error("expected some invalid entries, got none")
	}
	fmt.Println("Valid IPs:", valid)
	fmt.Println("Invalid entries:", invalid)
}
