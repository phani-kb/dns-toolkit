...
func generateSummariesReadme() string {
	...
	fmt.Fprintf(&sb, "**Last Generated:** %s\n\n", info.LastGenerated)

	// Quick overview
	sb.WriteString("## Overview\n\n")
	sb.WriteString("| Metric | Count |\n")
	sb.WriteString("|--------|-------|\n")
	fmt.Fprintf(&sb, "| Total Summary Files | %d |\n", info.TotalFiles)
	fmt.Fprintf(&sb, "| Summary Types | %d |\n", len(info.SummaryTypes))
	sb.WriteString("\n")
	...
	fmt.Fprintf(&sb, "These summaries are automatically generated by the [DNS Toolkit](%s) ",
...
func getDetailedStatsForSummaryType(filename string) string {
	...
	case "download_summary.json":
		...
		fmt.Fprintf(&result, "**Sources:** %d total, %d successful, %d failed  \n",
	...
	case "processed_summary.json":
		...
		var result strings.Builder
		fmt.Fprintf(&result, "**Sources:** %d processed  \n", len(processedSummaries))
		fmt.Fprintf(&result, "**Files:** %d valid, %d invalid  \n", totalValid, totalInvalid)
	...
	case "consolidated_summary.json":
		...
		var result strings.Builder
		fmt.Fprintf(&result, "**Files:** %d consolidated  \n", totalFiles)
		fmt.Fprintf(&result, "**Entries:** %s total  \n", formatNumber(totalEntries))
	...
	case "top_summary.json":
		...
		var result strings.Builder
		fmt.Fprintf(&result, "**Types:** %d analyzed  \n", len(topSummaries))
		fmt.Fprintf(&result, "**Top Entries:** %s total  \n", formatNumber(totalEntries))
	...
	case "overlap_summary.json":
		...
		var result strings.Builder
		fmt.Fprintf(&result, "**Sources:** %d analyzed  \n", len(overlapSummaries))
		fmt.Fprintf(&result, "**Total Entries:** %s  \n", formatNumber(totalEntries))
		fmt.Fprintf(&result, "**Unique Entries:** %s (%.1f%%)  \n", formatNumber(totalUnique), uniquePercentage)
...
func summarizeConsolidatedSummaries(summaries []c.ConsolidatedSummary, field string) string {
	...
	var result strings.Builder
	if field == "category" {
		fmt.Fprintf(&result, "**Categories:** %d processed  \n", len(countMap))
	} else {
		fmt.Fprintf(&result, "**Groups:** %d processed  \n", len(countMap))
	}
	fmt.Fprintf(&result, "**Files:** %d consolidated  \n", totalFiles)
	fmt.Fprintf(&result, "**Entries:** %s total  \n", formatNumber(totalEntries))
