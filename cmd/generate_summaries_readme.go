...
func generateSummariesReadme() string {
	...
	_, _ = fmt.Fprintf(&sb, "**Last Generated:** %s\n\n", info.LastGenerated)

	// Quick overview
	sb.WriteString("## Overview\n\n")
	sb.WriteString("| Metric | Count |\n")
	sb.WriteString("|--------|-------|\n")
	_, _ = fmt.Fprintf(&sb, "| Total Summary Files | %d |\n", info.TotalFiles)
	_, _ = fmt.Fprintf(&sb, "| Summary Types | %d |\n", len(info.SummaryTypes))
	sb.WriteString("\n")
	...
	sb.WriteString("|---------|-------|\n")
	_, _ = fmt.Fprintf(&sb, "| Sources Configured | %d |\n", info.OverallStats.TotalSources)
	_, _ = fmt.Fprintf(&sb, "| Downloads Attempted | %d |\n", info.OverallStats.TotalDownloads)
	_, _ = fmt.Fprintf(&sb, "| Files Processed | %d |\n", info.OverallStats.TotalProcessed)
	_, _ = fmt.Fprintf(&sb, "| Files Consolidated | %d |\n", info.OverallStats.TotalConsolidated)
	_, _ = fmt.Fprintf(&sb, "| Groups Generated | %d |\n", info.OverallStats.TotalGroups)
	_, _ = fmt.Fprintf(&sb, "| Categories Generated | %d |\n", info.OverallStats.TotalCategories)
	_, _ = fmt.Fprintf(&sb, "| Top Lists Generated | %d |\n", info.OverallStats.TotalTopLists)
	_, _ = fmt.Fprintf(&sb, "| Sources Analyzed for Overlap | %d |\n", info.OverallStats.TotalOverlapAnalyzed)
	sb.WriteString("\n")
...
func getDetailedStatsForSummaryType(filename string) string {
	...
	case "download_summary.json":
		...
		_, _ = fmt.Fprintf(&result, "**Sources:** %d total, %d successful, %d failed  \n",
	...
	case "processed_summary.json":
		...
		var result strings.Builder
		_, _ = fmt.Fprintf(&result, "**Sources:** %d processed  \n", len(processedSummaries))
		_, _ = fmt.Fprintf(&result, "**Files:** %d valid, %d invalid  \n", totalValid, totalInvalid)
	...
	case "consolidated_summary.json":
		...
		var result strings.Builder
		_, _ = fmt.Fprintf(&result, "**Files:** %d consolidated  \n", totalFiles)
		_, _ = fmt.Fprintf(&result, "**Entries:** %s total  \n", formatNumber(totalEntries))
	...
	case "top_summary.json":
		...
		var result strings.Builder
		_, _ = fmt.Fprintf(&result, "**Types:** %d analyzed  \n", len(topSummaries))
		_, _ = fmt.Fprintf(&result, "**Top Entries:** %s total  \n", formatNumber(totalEntries))
	...
	case "overlap_summary.json":
		...
		var result strings.Builder
		_, _ = fmt.Fprintf(&result, "**Sources:** %d analyzed  \n", len(overlapSummaries))
		_, _ = fmt.Fprintf(&result, "**Total Entries:** %s  \n", formatNumber(totalEntries))
		_, _ = fmt.Fprintf(&result, "**Unique Entries:** %s (%.1f%%)  \n", formatNumber(totalUnique), uniquePercentage)
...
func summarizeConsolidatedSummaries(summaries []c.ConsolidatedSummary, field string) string {
	...
	var result strings.Builder
	if field == "category" {
		_, _ = fmt.Fprintf(&result, "**Categories:** %d processed  \n", len(countMap))
	} else {
		_, _ = fmt.Fprintf(&result, "**Groups:** %d processed  \n", len(countMap))
	}
	_, _ = fmt.Fprintf(&result, "**Files:** %d consolidated  \n", totalFiles)
	_, _ = fmt.Fprintf(&result, "**Entries:** %s total  \n", formatNumber(totalEntries))
