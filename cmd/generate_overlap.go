...
var generateOverlapCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := os.WriteFile(overlapPath, []byte(overlapMd), 0o644); err != nil {
...
func generateDetailedOverlapAnalysis() (string, error) {
	...
	_, _ = fmt.Fprintf(&sb, "| Total Entries Analyzed | %s |\n", formatNumber(totalEntries))
	sb.WriteString("\n")
	...
	for _, summary := range overlapSummaries {
		...
		_, _ = fmt.Fprintf(&sb, "<summary>%s</summary>\n\n", concise)
		...
		if len(summary.TargetsList) > 0 {
			...
			for _, targetStr := range summary.TargetsList {
				...
				if target.OverlapCount > 0 || target.OverlapPercent >= minPercent {
					_, _ = fmt.Fprintf(&sb, "| %s | %s | %s | %s | %s | %.1f%% |\n",
						target.Name,
						target.ListType,
						target.SourceType,
